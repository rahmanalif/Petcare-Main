...
export const searchWalkingSitters = createAsyncThunk(
  "walkingSearch/searchWalkingSitters",
  async (payload, { rejectWithValue }) => {
    try {
      const params = new URLSearchParams();
      params.set("serviceType", "walking");
      ...
      if (filters.certifiedFirstAid === true) {
        params.set("certifiedFirstAid", "true");
      }
      if (filters.canDoBathingGrooming === true) {
        params.set("canDoBathingGrooming", "true");
      }
      ...
      if (!response.ok) {
        return rejectWithValue(result?.message || "Walking search failed");
      }
      ...
      return rejectWithValue(error?.message || "Walking search failed");
    }
  }
);

const walkingSearchSlice = createSlice({
  name: "walkingSearch",
  initialState: {
    results: [],
    loading: false,
  ...
  extraReducers: (builder) => {
    builder
      .addCase(searchWalkingSitters.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(searchWalkingSitters.fulfilled, (state, action) => {
        state.loading = false;
        state.results = action.payload;
      })
      .addCase(searchWalkingSitters.rejected, (state, action) => {
        state.loading = false;
        state.results = [];
        state.error = action.payload || "Walking search failed";
      });
  },
});

export default walkingSearchSlice.reducer;
